...
public class Camera 
{
	...
	static double cameraX = -5000;
	static double cameraY = -5000;
	public static double playerSpeed = 15;
	...
	public void tick()
	{
		if(!KeyManager.itemUse)
		{
			...
			else if(KeyManager.up && !Player.upLock && !Collision.upLock && ! FlatCollision.upLock)
			{
				moveUp();
			}
			
			if(KeyManager.down && Player.player.y < Handler.height / 2 && !Player.downLock && !Collision.downLock && ! FlatCollision.downLock)
			{
				moveDownScreen();
			}
			else if(KeyManager.down && cameraY >= -(Maps.mapHeight * Maps.tileScale - Handler.height - 5) && !Player.downLock && !Collision.downLock && ! FlatCollision.downLock)
			{
				moveDown();
			}
			...
			else if(KeyManager.left && Player.playerPosMem.x> Handler.width / 2 && !Player.leftLock && !Collision.leftLock && ! FlatCollision.leftLock)
			{
				moveLeft();
			}
			...
			else if(KeyManager.right && ! Player.rightLock && !Collision.rightLock && ! FlatCollision.rightLock)// && cameraX >= -(Maps.mapWidth * Maps.tileScale - Handler.width - 5))
			{
	...
	public void setPlayer(int x, int y)
	{
		...
		{
			Handler.util.chatMsg(Color.RED, "Out of Bounds : ", "Tried to teleport out of bounds : " + x, 300);
		}
	}
}
