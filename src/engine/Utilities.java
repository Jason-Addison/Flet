...
public class Utilities {
	public Point2D.Double getMouse() {
		Point2D.Double mouse = new Point2D.Double(Handler.mouseX + Player.playerPosMem.x - Player.player.x,
				Handler.mouseY + Player.playerPosMem.y - Player.player.y);
		return mouse;
	}
	...
	public int randomNumber(int max, int min) {
		if (max > min) {
			...
			chatMsg(Color.BLACK, "Warning ! : ",
					new String("The minimum random number was greater than the maximum! Max : " + max + " Min : " + min
							+ " (Returned minimum : " + min + ")"),
					500);
		}
		return min;
	}
	...
	public void setMouse() {
		...
		Cursor c = toolkit.createCustomCursor(Sources.MOUSE_NOINTERACT, new Point(Game.frame.getX(), Game.frame.getY()),
				"img");
	...
	public void addEntity(Entity entity, Double x, Double y) {
		...
		Handler.chunk.addEntity(xPos, yPos, entity);
		System.out
				.println("Entity added : " + entity.getClass().getSimpleName() + " at x : " + x + " y : " + y + "  |");// Size
																														// :
																														// "
																														// +
																														// Handler.world.map[xPos][yPos].entities.size());
	}
	...
	public double removeDecimalOLD(double number) {
		String stringNumber = Double.toString(number);
		String stringNoDecimal = stringNumber.split("\\.", 2)[0];
		return Double.parseDouble(stringNoDecimal);
	}

	public Object JSONToObject(Object source, Object convertedClass) {
		Object newObject = new Gson().fromJson(source.toString(), convertedClass.getClass());
		return newObject;
	}
	...
	public void screenCapture(String path, String name) {
